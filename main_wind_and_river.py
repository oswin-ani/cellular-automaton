            ...
            for x in range(SCREEN_WIDTH // BLOCK_SIZE):
                if fair_rects[y][x] > 0:
                    ...
                    for dx,dy in neighbourhood:
                        if ((dx == 1 and dy == 0) or (dx == 1 and dy == 1) or (dx == 0 and dy == 1)) and random.random() < 0.34:
                            continue
    ...
    for y in range(SCREEN_HEIGHT // BLOCK_SIZE):
        for x in range(SCREEN_WIDTH // BLOCK_SIZE):
            if (x,y) not in soil_rects:
                ...
                if fair_rects[y][x] > 80 and random.random() < forest_fraction:
                    pygame.draw.rect(frame, TREE, (x * BLOCK_SIZE, y * BLOCK_SIZE, BLOCK_SIZE - 1, BLOCK_SIZE - 1))
            else:
